...
# Absolute imports for config and vectorstore
from agentbot.backend.config import GROQ_API_KEY, TAVILY_API_KEY
from agentbot.backend.vectorstore import get_retriever

# --- Tools ---
if TAVILY_API_KEY:
    os.environ["TAVILY_API_KEY"] = TAVILY_API_KEY
else:
    print("Warning: TAVILY_API_KEY is not set. Web search tool may fail if used.")
...
if GROQ_API_KEY:
    ...
    # Fallback dummy LLMs for local UI/testing when GROQ_API_KEY is not provided.
    print("Warning: GROQ_API_KEY is not set. Using dummy LLM fallbacks for local testing.")

    class _DummyRouter:
        def with_structured_output(self, schema):
            return self
        def invoke(self, messages):
            # Default to a safe 'answer' route
            class R:
                pass
            r = R()
            r.route = "answer"
    ...
    class _DummyAnswer:
        def invoke(self, messages):
            class M:
                pass
            m = M()
            m.content = "(LLM disabled - running with dummy keys)"
            return m

    router_llm = _DummyRouter()
    judge_llm = _DummyJudge()
    answer_llm = _DummyAnswer()
